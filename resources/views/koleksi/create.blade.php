...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('koleksi.store') }}" method="POST" enctype="multipart/form-data" class="form">
            ...
            <div class="form-group mb-3">
                <label for="tipe">Tipe Koleksi</label>
                <select class="form-select" name="tipe" id="tipe">
                    <option selected>-- Pilih tipe --</option>
                    <option value="modul"
                        {{ old('tipe') == 'modul' ? 'selected' : ''}}>
                        Modul
                    </option>
                    <option value="buku"
                        {{ old('kategori') == 'buku' ? 'selected' : ''}}>
                        Buku
                    </option>
                    <option value="ppt"
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-select" name="mapel" id="mapel">
                    ...
                    <option value="ipa"
                        {{ old('kelas') == 'ipa' ? 'selected' : ''}}>
                        Ilmu Pengetahuan Alam
                    </option>
                    <option value="ips"
                        {{ old('kelas') == 'ips' ? 'selected' : ''}}>
                        Ilmu Pengetahuan Sosial
                    </option>
                    <option value="matematika"
                        {{ old('kelas') == 'matematika' ? 'selected' : ''}}>
                        Matematika
                    </option>
                </select>
                @error('mapel')
